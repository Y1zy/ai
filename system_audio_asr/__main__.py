...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--share-lan", action="store_true",
                        help="监听局域网（手机扫码投屏需要）；也可在设置页勾选启用后重启")
...
def main() -> None:
    ...
    host = args.host
    if host == "127.0.0.1":
        from .phone_share import load_phone_config

        if args.share_lan or load_phone_config().get("enabled"):
            host = "0.0.0.0"
            print(
                "手机投屏已启用：服务监听局域网，二维码见 http://127.0.0.1:%d/settings" % args.port
            )
