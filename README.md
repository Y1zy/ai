...
- **📋 简历上下文**：填入简历全文、目标 JD、公司、附加背景后，AI 回答会直接引用候选人的真实经历，而不是泛泛而谈——面试时问「讲讲你最挑战的项目」，AI 会基于简历里的项目细节来答。
- **🫥 共享屏幕隐形**：字幕悬浮窗默认从所有屏幕采集路径中消失——腾讯会议 / Zoom / Teams 共享屏幕、录屏软件、截图工具都看不到它，但你自己屏幕上照常显示（Windows 10 2004+ 的 `WDA_EXCLUDEFROMCAPTURE`）。录制演示视频时可用 `.\start_overlay.ps1 -AllowCapture` 恢复可见。
- **📱 手机投屏**：设置页出示二维码，手机扫码直连电脑（局域网，本地直连不经服务器），在手机上查看并保存电脑屏幕截图、查看实时字幕与 AI 流式回答；支持手机↔电脑剪贴板双向同步。
- **🔒 极致隐私**：仅在本地运行 ASR 引擎，只有定稿后的文本才会发送给 AI。按 `Ctrl+Alt+H` 会隐藏悬浮窗（后台识别与 AI 回答继续，恢复后显示累积内容）；要连识别一起停下，用控制条的暂停按钮。
...
第一次启动会联网下载 Paraformer 模型，需要耐心等待。`launch.ps1` 会一次性启动 ASR 服务、手机投屏与桌面字幕悬浮窗；看到 `VoxRibbon 已就绪` 后，桌面会出现字幕区域。附加参数：`-NoPhone` 关闭手机投屏、`-AllowCapture` 录制演示时让字幕可见、`-AISettings` 直接打开 AI 设置。
...
老板键优先使用 `Ctrl+Alt+H`，冲突时自动回退为 `Ctrl+Shift+H`。按一次隐藏字幕（只隐藏窗口，识别、AI 回答与手机投屏都在后台继续），再按一次恢复并显示这期间累积的内容。底层 ASR 服务保持运行，避免恢复时重新加载模型。若要让识别也真正停下，用控制条的暂停按钮。

想让鼠标点击穿透到字幕后面的窗口（例如面试平台的输入框），按 `Ctrl+Alt+L`（冲突时 `Ctrl+Shift+L`）切换锁定；等效于鼠标悬停后点击控制条的小锁图标。锁定后字幕仍照常显示与识别。
...
「回答长度」档位（设置页与桌面设置窗均可配）控制单次回答的 token 上限，六档 256–8192，默认 `完整 — 约 2048`（即历史行为）。额度越大回答越完整，但简单题也更容易被展开成带 Markdown / 代码示例的长文；想要更克制的口语化短答可调低到 `适中` 或 `简洁`。对字幕 AI、手机追问与「回答测试」生效；截图解题固定 2048。手改 `config.json` 的任意值会吸附到最近档。
...
悬浮窗默认开启“共享屏幕隐形”：主字幕窗、编辑设置窗和位置锁提示窗都会调用 `SetWindowDisplayAffinity(WDA_EXCLUDEFROMCAPTURE)`，从所有屏幕采集路径中消失——腾讯会议 / Zoom / Teams 的屏幕共享、OBS 等录屏软件、`Win+Shift+S` 截图都看不到这些窗口，但你自己屏幕上照常显示。需要 Windows 10 2004 (build 19041) 及以上，启动日志会记录 `capture_protection=True applied=True` 表示生效。

- 需要录制演示视频或反馈问题时，用 `.\start_overlay.ps1 -AllowCapture` 启动（本次运行不隐藏，配置文件不受影响），或用 `.\start_overlay.ps1 -Edit` 打开设置窗后取消勾选“共享屏幕 / 录屏时隐藏本窗口”。
- 开关持久化在 `%LOCALAPPDATA%\WasapiParaformerOverlay\config.json` 的 `captureInvisible` 字段。
- 验证方法：Overlay 运行时执行 `.\overlay_cs\bin\OverlayCapture.exe out.png 0 --capture-affinity`，输出 `affinity=17` 表示已隐形，`affinity=0` 表示正常可见。

### 手机投屏（扫码配对）

电脑端可以生成二维码，手机扫码后在浏览器里直连电脑（局域网，不经任何服务器），查看**电脑屏幕截图**、实时字幕和 AI 回答——手机相当于一块不参与屏幕共享的私密屏幕。

1. 打开设置页 <http://127.0.0.1:8765/settings>，在“📱 手机投屏（扫码配对）”卡片勾选**启用手机投屏**。
2. 重启 `.\start.ps1`（或直接用 `.\start.ps1 -SharePhone` 启动），服务会以局域网模式监听；首次开放局域网时 Windows 防火墙弹窗请选择允许（专用网络）。
3. 刷新设置页，用手机相机扫描二维码（或复制链接到手机浏览器打开）。手机与电脑必须连到同一个网络，推荐手机开热点、电脑连接该热点。
4. 手机页面可以：**📸 立即截屏**、**💾 保存图片**（下载带时间戳的 JPG，也可长按图片保存）、开启自动刷新（3 秒）、查看实时字幕与 AI 流式回答。
5. **剪贴板双向同步**：电脑上 Ctrl+C 复制的文字会自动推送到手机页面（一键“复制到手机”）；手机页面输入或粘贴文字后点“发送到电脑”，电脑上直接 Ctrl+V 即可粘贴。

- 配对凭据保存在 `%LOCALAPPDATA%\WasapiParaformerOverlay\phone_share.json`；“重新生成配对码”会使旧二维码立即失效。
- 手机页面受会话 ID + 令牌双重校验，设置页等敏感接口仍然只允许本机访问。
- 截图由电脑端即时抓取（JPEG，质量 75，长边限 1600px），只推送给扫码手机，不落盘、不经过第三方。
- 剪贴板同步只在投屏启用且手机已连接时进行：电脑端通过 `GetClipboardSequenceNumber` 轮询（约 0.8 秒），手机推送的文字直接写入电脑剪贴板；超过 50000 字符会截断。
- 电脑上开启“共享屏幕隐形”的字幕悬浮窗不会出现在手机看到的截图里（同一套系统采集排除机制）。
